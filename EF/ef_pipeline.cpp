...
{
	vkDestroyShaderModule(efDevice.device(), vertShaderModule, nullptr);
	vkDestroyShaderModule(efDevice.device(), fragShaderModule, nullptr);
	vkDestroyPipeline(efDevice.device(), graphicsPipeline, nullptr);
}
...
PipelineConfigInfo EfPipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo, uint32_t width, uint32_t height)
{
...
void EfPipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& fragFilepath, const PipelineConfigInfo& configInfo)
{
	...
	auto bindingDescriptions   = EfModel::Vertex::getBindingDescriptions();
	auto attributeDescriptions = EfModel::Vertex::getAttributeDescriptions();
...
void EfPipeline::bind(VkCommandBuffer commandBuffer)
{
